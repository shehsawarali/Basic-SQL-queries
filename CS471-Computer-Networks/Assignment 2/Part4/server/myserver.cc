	...
	while(true)
	{
		...
		while(sentinel)
		{
			...
			else if(recvbuf[0] == 'c' && recvbuf[1] == 'r' && recvbuf[2] == 'e' && recvbuf[3] == 'a' && recvbuf[4] == 't' && recvbuf[5] == 'e')
			{
				...
				if(fileexist(file))
				{
					send(clientsock, "Server: The file already exists", 31, 0);
				}
				else
				{
					ofstream data;
	                ...
	                data.close();
	                send(clientsock, "Server: The file has been created", 33, 0);
				}
			}
			else if(recvbuf[0] == 'd' && recvbuf[1] == 'e' && recvbuf[2] == 'l' && recvbuf[3] == 'e' && recvbuf[4] == 't' && recvbuf[5] == 'e')
			{
				int i = 14; char file[BUFLEN] = {'\0'}; int count = 0;
				while(recvbuf[i] != '\0')
				{
					file[count] += recvbuf[i];
					i++;
					count++;
				}
				...
				if(fileexist(file))
				{
					...
					if(check == 0)
					{
						send(clientsock, "Server: The file has been deleted", 33, 0);
					}
					else
					{
						send(clientsock, "Server: Error. File not deleted", 31, 0);
					}
				}
				else
				{
	                send(clientsock, "Server: The file does not exist", 31, 0);
				}
			}
			else if(recvbuf[0] == 'r' && recvbuf[1] == 'e' && recvbuf[2] == 'c' && recvbuf[3] == 'e' && recvbuf[4] == 'i' && recvbuf[5] == 'v' && recvbuf[6] == 'e')
			{
				int i = 8; string file = "";
				while(recvbuf[i] != '\0')
				{
			...
			else if(recvbuf[0] == 's' && recvbuf[1] == 'e' && recvbuf[3] == 'n' && recvbuf[4] == 'd')
			{
				int i = 5; string file = "";
				while(recvbuf[i] != '\0')
				{
				...
				if(fileexist(file))
				{
					cout<<"Server: File already exists. Replace or keep the old copy?"<<endl;
				}
				else
				{
					send(clientsock, "Server: The file does not exist", 31, 0);
			...
			{
				send(clientsock, "Server: Command received", 24, 0);
			}
